...
Copyright (C) 2020, SUSE LLC. All rights reserved.<BR>
  ...
  )
{
  if (ImageUpdatable == NULL) {
    DEBUG((DEBUG_ERROR, "CheckImage - Invalid ImageUpdatable\n"));
    return EFI_INVALID_PARAMETER;
  }

  if (Image == NULL) {
    DEBUG((DEBUG_ERROR, "CheckImage - Invalid Image\n"));
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR, "SetImage - CheckImage failed: %r.\n", Status));
    goto cleanup;
  }

  if (Updatable != IMAGE_UPDATABLE_VALID) {
    DEBUG((DEBUG_ERROR, "SetImage - Invalid image for update: 0x%X.\n", Updatable));
    Status = EFI_ABORTED;
    goto cleanup;
  }
  ...
  Status = Progress(15);
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR, "SetImage - Progress failed: %r.\n", Status));
  }
